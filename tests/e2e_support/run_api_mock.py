...
- una DB descartable sembrada desde una copia (`E2E_SEED_DB`) → los tests no
  tocan datos reales y arrancan siempre del mismo estado;
- el LLM reemplazado por `MockLLMAdapter` con una demora por llamada
  (`E2E_LLM_DELAY`) para que el avance en vivo sea observable.

Variables: E2E_DB (destino), E2E_SEED_DB (origen, opcional),
E2E_LLM_DELAY (segundos, default 0.15), E2E_API_PORT (default 8021).
"""
...
seed_path = os.environ.get("E2E_SEED_DB")
...
if seed_path:
    with sqlite3.connect(seed_path) as src, sqlite3.connect(db_path) as dst:
        src.backup(dst)
