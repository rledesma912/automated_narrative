...
class TestSqlBeatRepository:
    ...
    @pytest.mark.asyncio
    async def test_save_beat(self, repo, setup_db, story_id):
        """Test saving a beat to database."""
        beat = Beat(number=1, summary="Beat summary")

        result = await repo.save(beat, story_id)

        assert result.number == 1
        assert result.summary == "Beat summary"

    @pytest.mark.asyncio
    async def test_get_by_story(self, repo, setup_db, story_id):
        """Test retrieving beats by story ID."""
        beat1 = Beat(number=1, summary="First beat")
        beat2 = Beat(number=2, summary="Second beat")

        await repo.save(beat1, story_id)
        await repo.save(beat2, story_id)

        results = await repo.get_by_story(story_id)
    ...
    @pytest.mark.asyncio
    async def test_get_by_number(self, repo, setup_db, story_id):
        """Test retrieving a specific beat by number."""
        beat = Beat(number=3, summary="Third beat")

        await repo.save(beat, story_id)
        result = await repo.get_by_number(story_id, 3)
    ...
    @pytest.mark.asyncio
    async def test_get_by_number_returns_none(self, repo, setup_db, story_id):
        """Test that get_by_number returns None for nonexistent."""
        result = await repo.get_by_number(story_id, 999)

        assert result is None

    @pytest.mark.asyncio
    async def test_update_beat(self, repo, setup_db, story_id):
        ...
        result = await repo.update(beat, story_id)
