...
- **Asistente de autoría (Spec-530):** «Nuevo relato» → `/nuevo` y `/asistente/{id}/{direccion|taller|escaleta}`. Guardado automático desde `public/js/asistente.js` directo a `/api/v1/authoring/*`; la IA solo con comandos explícitos (jobs `consult` / `plan_outline` / `verify_outline`), con un modal que bloquea la página hasta que el job termina. Los personajes se suman en el acto donde aparecen. En la galería y la ficha, «Editar» lleva al asistente si `story.authoring`.
