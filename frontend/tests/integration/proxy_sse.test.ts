 ...
 */
// Spec-460: la sala (`/api/v1/jobs/:id/events`) y el canal global (`/api/v1/events`)
// pasan por el mismo proxy; los dos tienen que llegar sin buffer.
describe.each(["/api/v1/jobs/abc/events", "/api/v1/events"])(
  "api proxy — SSE streaming integrity (%s)",
  (ssePath) => {
    it("streams 5 SSE events through the proxy without buffering", async () => {
      ...
      await new Promise<void>((resolve, reject) => {
        const req = http.request(
          { hostname: "127.0.0.1", port, path: ssePath, method: "GET" },
          (res) => {
