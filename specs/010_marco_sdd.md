...
| **Normalizer** | Elimina ruido de proceso LLM: bloques de razonamiento (`<think>`) y relleno conversacional. No altera Markdown válido del output. Ver Spec 044. | `ResponseNormalizer` |
...
1. Elimina bloques de razonamiento interno: `<think>`, `<thought>`, `<reasoning>` con su contenido
2. Filtra relleno conversacional: frases como "Aquí tienes", "Por supuesto", "Espero que te guste"
3. Preserva saltos de párrafo (`\n\n`) — no colapsa prosa narrativa
4. **No toca** estructura Markdown válida (headers, separadores, code fences) — eso es parte de la respuesta
5. Aplica `model_overrides` según substring del nombre del modelo activo
